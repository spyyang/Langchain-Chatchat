...
class CollectionStore(BaseModel):
    ...
    @classmethod
    def get_all_collection_names(cls, session: Session) -> List[str]:
        return [record.name for record in session.query(cls).all()]  # type: ignore
...
class AnalyticDB(VectorStore):
    ...
    def get_collections(self) -> List[str]:
        with Session(self._conn) as session:
            return CollectionStore.get_all_collection_names(session)
